...
- [ ] **Diferido — Timeline scroll-jacking (post-revert D28)** — 5 sprints (Pieza D base → wheel mejora → Pieza E v1 translateX → Pieza E v2 slots cinemáticos → calibración pinScroll → migración GSAP) intentaron implementar scroll-jacking pinneado para `s_avanzosc_timeline` (8 hitos). Bug visual «ghost» persistente: cadencia inter-idx 30-200ms vs duración mínima animación 0.5s = transitions interrumpidas-y-redirigidas. Causa raíz por física, no por bug. Revertido en commit `827ec78` — timeline vuelve al original (8 hitos visibles en flex horizontal con scroll-x nativo). Trigger reapertura: si se aborda timeline visual «impresionante», considerar approaches alternativos al scroll-jacking sobre N hitos discretos (carrusel swipe nativo Pieza D, stagger reveal one-shot, vertical column timeline, o reducir a 3-4 hitos consolidados con copy revisado). NO bloquea switchover. Ver [decisions-log §6 deferred-timeline-scrolljacking](docs/decisions-log.md#deferred-timeline-scrolljacking).
