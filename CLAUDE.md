...
**Manifest `assets` cacheado por el dev server v18 — restart obligatorio al AÑADIR archivos al dict.** El proceso del dev server lee los manifests al arrancar y los cachea. Tras añadir un archivo nuevo al dict `assets` del `__manifest__.py` y correr `-u` (proceso aparte), el `-u` invalida los attachments del bundle, pero el PRIMER request HTTP los regenera usando el manifest CACHEADO del server en marcha → bundle sin el archivo nuevo, sin error ni warning. Síntoma: estilos/JS del archivo nuevo no aplican aunque el smoke pase (descubierto con `_kit_page.scss`, tanda 2 UI-1 2026-06-11). **Patrón**: añadir al manifest → `-u` → RESTART del dev server → warm request. NO aplica al editar contenido de archivos ya registrados (el bundle se regenera del disco). Variante v18 del gotcha «bundle se invalida con -u» de esta sección.

**Herencias sobre `t-set` de QWeb pueden quedar inertes tras migrar de versión sin dar error.** Una herencia `<xpath expr="//t[@t-set='var']" position="replace">` solo es útil mientras el core CONSUMA esa variable. En v18 las vars `cta_btn_href`/`cta_btn_text` siguen declaradas en `website.layout` (la herencia aplica limpia, sin ParseError) pero ya nada las lee: el CTA del header es un `<a href="/contactus">` hardcodeado en `website.header_call_to_action` (placeholder pattern). Síntoma: el override carga, el smoke pasa, y el botón sigue siendo el default del core («Contáctenos» → /contactus, regresión F3 detectada en auditoría UI 2026-06-11, fix T1.1). **Patrón de verificación**: tras migrar una herencia, verificar el RENDER (curl/DOM), no solo que la vista cargue. Aplica a cualquier override de `t-set` heredado de v14.
...
**SVG global con `viewBox` fijo + `preserveAspectRatio` (`slice` o `none`) NO sirve para anotaciones que deben aparecer en posiciones fijas respecto al container.** El viewBox tiene aspect ratio fijo; el container puede tener cualquier viewport ratio. Cuando ratios divergen: con `none` el viewBox se estira no-uniformemente y el texto/formas se distorsionan; con `slice` el viewBox se recorta a los bordes para llenar — los elementos posicionados en X o Y extremos del viewBox DESAPARECEN según viewport (visitantes en distintas resoluciones ven distintas anotaciones). Síntoma empírico cerrando D29 hero v3: viewBox `1400×600` + `slice` en mobile `375×480` recortaba a x∈[465, 935] aprox; anotaciones en X<465 invisibles en mobile aunque CSS las marcara visibles. **Fix por construcción**: anotaciones fixed-position se implementan en HTML (un `<div class="annotation">` por elemento) + CSS `position: absolute` con `top/left/right/bottom` en %. Mini-SVGs locales pequeños (viewBox propio reducido) sólo para los gráficos vectoriales — NUNCA un SVG global con viewBox para layout. Texto siempre en HTML (`<span>`); excepción aceptable: labels inline a un gráfico vectorial (axis XYZ, etc.) que viven dentro del mini-SVG local. **Cuándo NO aplica**: el SVG global con viewBox + preserveAspectRatio sigue siendo correcto para curvas/paths que SÍ deben escalar uniformemente con el container (ej. `s_avanzosc_hero_lines` con sus 3 trazos blueprint que cubren todo el ancho del hero). La regla aplica solo a anotaciones fixed-position. Verificado empíricamente cerrando `D29 Pieza A v3` (decisions-log §5) — descartados 2 approaches SVG (preserveAspectRatio `none` y `slice`) antes del HTML+CSS positioned.
...
| D29 | Pieza A v3 hero decoración enriquecida — 10 anotaciones tipo CAD HTML+CSS positioned + hover individual + mobile dedicado | [link](docs/decisions-log.md#d29) |
