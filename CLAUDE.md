...
**`scrollLeft += delta` no funciona con `scroll-snap-type: x mandatory`.** El navegador snap revierte cualquier escritura de scroll que no caiga en un snap point. Si el delta es menor que la distancia entre snaps (~stride entre items), `scrollLeft` queda atrapado en el snap actual sin avanzar. Solución snap-aware: `element.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' })` sobre el item destino — el browser respeta el snap y anima el scroll. Verificado empíricamente Pieza D mejora wheel sesión 2026-05-05.

**Lenis 1.0.42 SÍ soporta `data-lenis-prevent-wheel`** como atributo HTML en wrapper o ancestor del wheel target. Cuando Lenis recibe wheel y el target o cualquier ancestor tiene ese atributo, skipea el procesamiento (no scrollea wrapwrap). Útil para zonas con scroll lateral propio (carrusel) que no deben triggear scroll vertical de la página. Verificable en source `cdn.jsdelivr.net/npm/@studio-freight/lenis@1.0.42/dist/lenis.js`. Información previa errónea (afirmaba que NO soportaba) corregida 2026-05-05.

**Chrome/Webkit `scrollWidth` excluye `padding-right`** en contenedores `overflow-x: auto`. Cuando un track horizontal tiene `padding: 0 calc(50% - itemWidth/2)` para centrar primer/último item, `maxScrollLeft = scrollWidth - clientWidth` queda corto en una cantidad igual al padding-right (Chrome cap el scroll a la posición donde el último item empieza, no donde podría centrarse). Síntoma empírico Pieza E v1: `maxScrollLeft = 1713` vs requerido 2128 para centrar item 7 (~178px desfase visual). **Soluciones**: (a) animar `transform: translateX` directo en lugar de scroll mecánico, evitando los limits del navegador; (b) abandonar el track contínuo y usar slots fijos absolute (Pieza E v2 path).

**`gsap.from(elem, {opacity: 0, y: 20, ...})` deja inline styles** (transform/opacity) tras la animación que ganan por specificity sobre clases CSS de estado (1,0,0 inline > 0,0,3,0 selector). Si una pieza usa toggling de clases (como `setActiveTriad` en Pieza E v2 que aplica `is-slot-center` con `opacity: 1` esperado del CSS), el inline style residual del `gsap.from` previo bloquea el cambio. **Soluciones**: `clearProps: 'all'` en el tween de gsap.from, o skip del gsap.from cuando el modo de estado-clase está activo. Capturado en Pieza E v2 (skip reveal stagger cuando `pinEnabled`).

**axe-core 4.x factoriza `opacity` en cálculo color-contrast.** Color con ratio teórico OK puede fallar AA si el elemento o cualquier ancestor tiene `opacity < 1`. axe computa el color efectivo blend = `color × alpha + background × (1 − alpha)` y mide contrast del resultado vs el background. Las opacities heredan multiplicativamente: padre `opacity:0.5` × hijo `opacity:0.85` = combined alpha `0.425`. Ejemplo Pieza E v2: `--neutral-700` (#3A4149) puro vs blanco da 10.5:1 ✓; con `opacity:0.5` cae a 2.65:1 ✗. **Conclusión**: para texto WCAG-strict, diferenciar protagonismo por scale, posición o tamaño tipográfico — NO por opacity. Capturado en Pieza E v2 fix Opción 5 sesión 2026-05-05.
...
| D27 | Pieza A: hero decoration technical blueprint (grid de puntos en periferia + 3 líneas geométricas asimétricas con parallax). CSS radial-gradient + mask para dots; SVG inline con `vector-effect="non-scaling-stroke"` para líneas. Parallax vanilla scroll + rAF bound a `#wrapwrap` (precedente `timeline.js` descarta ScrollTrigger por overhead Lenis). Mobile: solo curva principal + dots con densidad reducida. LCP comparativo dentro tolerancia (-0.2s desktop, +0.3s mobile). | [decisions-log#d27](docs/decisions-log.md#d27) |
| D28 | Pieza E v2: timeline scroll-jacking pinneado con 3 slots cinemáticos (left preview / center protagonista / right preview). Bridge Lenis ↔ ScrollTrigger 3.12.5 (scrollerProxy + lenis.on('scroll', ST.update) + ST.defaults scroller wrapwrap). `setActiveTriad(idx)` aplica clases CSS de estado (is-hidden-left/slot-left/slot-center/slot-right/hidden-right) a los 8 items; CSS transitions 0.5s `cubic-bezier(0.45,0,0.55,1)` interpolan entre estados (idempotentes vs GSAP timelines, robustas frente a click salto lejano). Mobile y reduced-motion mantienen Pieza D base (scroll-snap horizontal nativo). Slots laterales: `--neutral-700` SIN opacity (Opción 5 fix WCAG AA — axe-core factoriza opacity en blend), diferenciación por scale 0.85 + posición lateral + tamaño tipográfico. ScrollTrigger reintroducido tras retirada en 61600ff (consumidor real). LCP dentro tolerancia (-100ms desktop, +200-300ms mobile variance). | [decisions-log#d28](docs/decisions-log.md#d28) |
