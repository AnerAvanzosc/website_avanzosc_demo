...
**1.5 — `views/layout.xml` footer.** Files: modify `views/layout.xml`. Ref: spec §8.2. Acceptance: xpath `//footer` reemplaza con 3 columnas + barra. COL1=Avanzosc(Conócenos/Trabaja/Contacto). COL2=Soluciones(4 sectoriales). COL3=Contacto(`tel:+34943026902`, `mailto:comercial@avanzosc.es`, dirección). Barra: `© 2026 Avanzosc S.L. · CIF B20875340 · Aviso legal · Política de privacidad · Cookies`. Links legales a `/aviso-legal`, `/politica-privacidad`, `/politica-cookies` (404 hasta Phase 6 — esperado). Slugs cortos sin «-de-» reconciliados con footer y plan en sesión Phase 6 (decisión G). Datos según CLAUDE.md §11 "Datos legales del footer (vigentes)". Deps: 1.1.
...
## Phase 7.5 — Deudas técnicas acumuladas

3 deudas detectadas durante Phases 4-6 + cookies fix sesión 2026-04-29 que NO entran en el cierre v1 funcional pero deben resolverse antes del switchover Phase 10. Decisión sesión 2026-04-29 (Phase 7 discovery): NO embeberlas en Phase 7 (rompe scope «mobile header»); aislarlas como sub-fase propia.

**7.5.1 — Slugs URL EU directos (`website.rewrite` 301).** Files: modify `data/redirects.xml` (añadir 10 entradas). Ref: spec D2, [?] #1 spec, hallazgo Phase 5/6. Acceptance: `<record model="website.rewrite">` con `redirect_type=301` por cada slug EU validado/draft → ES source. Lista: `/eu/industriala`→`/industrial`, `/eu/banaketa`→`/distribucion`, `/eu/zerbitzuak`→`/servicios`, `/eu/akademiak`→`/academias`, `/eu/ezagutu-gaitzazu`→`/conocenos`, `/eu/lan-egin-gurekin`→`/trabaja-con-nosotros`, `/eu/kontaktua`→`/contacto`, `/eu/lege-oharra`→`/aviso-legal`, `/eu/pribatutasun-politika`→`/politica-privacidad`, `/eu/cookien-politika`→`/politica-cookies`. Ningun rewrite para `/eu/kit-consulting` (D5: ES-only). El usuario que escriba `/eu/industriala` cae a `/industrial` con 301; combinarlo con `lang=eu_ES` requiere también prefijo `/eu_ES/` o `?lang=eu_ES`. **Aceptación matizada**: el rewrite hace que `/eu/industriala` SÍ resuelva (200 tras 301), pero el lang del response viene del cookie `frontend_lang` o del prefijo `/eu_ES/`. Para UX completa real recomendar combinar con click en switcher (cookie set) — diferir a Phase 7.5.4 si el usuario pide alias completo. Deps: Phase 7. **Bloqueo:** Q1 (gate validación slugs EU lingüística por Avanzosc).

**7.5.2 — Migrar 10 páginas Phase 5-6 al patrón split `<data>` blocks.** Files: modify `views/pages/industrial.xml`, `views/pages/distribucion.xml`, `views/pages/servicios.xml`, `views/pages/academias.xml`, `views/pages/conocenos.xml`, `views/pages/trabaja_con_nosotros.xml`, `views/pages/contacto.xml`, `views/pages/kit_consulting.xml`, `views/pages/legal_aviso.xml`, `views/pages/legal_privacidad.xml`. **`views/pages/home.xml` EXCLUIDA** — descubrimiento sesión 2026-04-29 7.5.2: `home.xml` usa `inherit_id="website.homepage"` + xpath replace contra la view core, NO crea un record `website.page` propio. Las views heredadas se recomponen en cada `-u` por el motor de herencia de Odoo, no hay deuda de arch_db que corregir; aplicar el wrapper sería cosmético sin efecto. Ref: hallazgo cookies fix sesión 2026-04-29 (commit `a435b0a`). Acceptance: cada file de los 10 pasa de un único `<data noupdate="1">` a dos blocks separados — `<data>` (sin noupdate) wrapping el `<template>`, `<data noupdate="1">` wrapping el `<record model="website.page">`. Patrón estándar Odoo. Permite que `-u` actualice arch_db tras edits sin intervención manual via shell. Para records ya existentes en BD con `ir.model.data.noupdate=True`, también ejecutar `UPDATE ir_model_data SET noupdate=False WHERE module='website_avanzosc_demo' AND model='ir.ui.view'` o el equivalente vía hook one-shot. Deps: Phase 7.

**7.5.3 — Refactor `s_avanzosc_caso_exito` archetype dict → `<t t-if/t-elif>`.** Files: modify `views/snippets/caso_exito.xml`. Ref: hallazgo Phase 5 (mismo pattern aplicado a `s_avanzosc_sector_specifics`). Acceptance: el bloque QWeb que recupera `archetypes[archetype_id]` se reescribe a `<t t-if archetype_id == '1'>...</t><t t-elif archetype_id == '2'>...</t>` con cada string como texto contenido en nodo (h2/p/li). Eliminar el `<t t-set archetypes t-value="{...}"/>`. Resultado: las ~80 strings de los 8 archetypes ahora extraíbles por i18n y traducibles. Añadir entradas correspondientes en `i18n/eu.po` solo para el archetype activo en home (cf. config_parameter `featured_archetype_id`). Los demás archetypes quedan sin EU draft hasta que se decida cuáles publicar realmente — decisión [?] #7 spec aún pendiente; trabajo sin gate adicional. Deps: 7.5.2 (patrón split ya aplicado al fichero) o independiente si se aplica antes. **Bloqueo blando:** [?] #7 spec (selección final archetype destacado).

**7.5.4 — Smoke + gate.** Acceptance: smoke clean. `curl -I http://localhost:14070/eu/industriala` 301 → `/industrial` 200. Deps: 7.5.1, 7.5.2, 7.5.3.
...
**9.7 — Refactor sticky header `padding` transition (deuda Phase 8.3).** **Estado: DIFERIDO A v2 (D3 cerrada sesión 2026-04-29 Phase 9.7).** Files: modify `static/src/scss/snippets/_header.scss`. Ref: hallazgo Phase 8.3 audit estático sesión 2026-04-29. Acceptance: el sticky header (`header { transition: padding-top, padding-bottom 250ms ... }`) es la ÚNICA animación layout-property del módulo. Decisión Phase 1.3 lo documentó como excepción explícita per CLAUDE.md §5 («padding y box-shadow son aceptables como excepciones específicas y limitadas porque (a) la transición es de 250ms, (b) la altura final cambia ~16px sin layout shift propagado, (c) la alternativa transform: scaleY introduciría distorsión»). Phase 8.3 audit confirmó que esta es la única excepción y aporta ~22 layout events/sec durante scroll active del estado scrolled (medido vía CDP Tracing). Migrar a un patrón GPU-only requiere repensar la estructura interna del navbar y la compensación del box-shadow — trabajo NO trivial con riesgo cruzado (sticky behavior + navbar-collapse mobile + 3 transitions co-localizadas). **Decisión condicional D3 sesión 2026-04-29**: refactor in situ SOLO si Phase 9 QA visual detecta artefactos del sticky transition (jitter, layout shift visible). Las 39 capturas Phase 9.1+9.2 mostraron sticky header funcionando limpiamente en los 3 viewports sin artefactos visibles → **NO se ejecuta el refactor en v1**. Documentado en CLAUDE.md §11 «Decisiones pendientes» como deuda formal v2. Deps: 9.6.
