...
**7.5.2 — Migrar 6 páginas Phase 4-6 al patrón split `<data>` blocks.** Files: modify `views/pages/home.xml`, `views/pages/industrial.xml`, `views/pages/distribucion.xml`, `views/pages/servicios.xml`, `views/pages/academias.xml`, `views/pages/conocenos.xml`, `views/pages/trabaja_con_nosotros.xml`, `views/pages/contacto.xml`, `views/pages/kit_consulting.xml`, `views/pages/legal_aviso.xml`, `views/pages/legal_privacidad.xml`. Ref: hallazgo cookies fix sesión 2026-04-29 (commit `a435b0a`). Acceptance: cada file pasa de un único `<data noupdate="1">` a dos blocks separados — `<data>` (sin noupdate) wrapping el `<template>`, `<data noupdate="1">` wrapping el `<record model="website.page">`. Patrón estándar Odoo. Permite que `-u` actualice arch_db tras edits sin intervención manual via shell. Para records ya existentes en BD con `ir.model.data.noupdate=True`, también ejecutar `UPDATE ir_model_data SET noupdate=False WHERE module='website_avanzosc_demo' AND model='ir.ui.view'` o el equivalente vía hook one-shot. Deps: Phase 7.
